    ...
    <header className="sticky top-0 z-40 border-b border-clay/70 bg-sand/85 backdrop-blur">
      ...
      <div className="shell flex h-16 items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href="/cart" className="relative inline-flex items-center gap-2 rounded-full border border-clay bg-white px-4 py-2 text-sm font-semibold transition-transform hover:scale-105 hover:border-ink/30">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            {count > 0 && (
              <span key={count} className="animate-pop absolute -right-1.5 -top-1.5 flex h-5 min-w-5 items-center justify-center rounded-full bg-accent px-1 text-xs font-bold text-white">
                {count}
              </span>
            )}
          </Link>
